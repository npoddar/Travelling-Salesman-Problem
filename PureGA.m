% To run this file, just type the file name 'PureGA' at the command
% prompt 
% This is the Pure Genetic Algorithm implementation for TSP
...
filename = strcat('Cities', int2str(n_cities), '.txt');
disp(filename);

%fileID = fopen('Cities200.txt','w');
dlmwrite(filename, cities, 'delimiter', '\t');
%fprintf(fileID,cities);

fprintf(1, 'Cities\n');
disp(cities);
fprintf(1, 'End Cities List');
...
% This section is for the Hybrid GA-I
...
% This section is for the Hybrid GA -II
...
% The best fitness is stored as the best_fitness for that generation
    ...
    title(['Best Pure GA Route (distance = ' num2str(best_fitness(iter)) ')'])
    figure(4)
    plot(best_fitness(1:iter), 'b', 'LineWidth', 2)
    title('Best Fitness - Pure GA')
    xlabel('Generation')
    ylabel('Distance')
