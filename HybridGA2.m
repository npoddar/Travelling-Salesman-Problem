% To run this file, just type the file name 'HybridGA2' at the command
% prompt 
% This is the Hybrid Genetic Algorithm-2.
% The initial population of the GA is completely (100%) filled with the top
% routes from the Nearest Neighbor Algorithm solutions
...
%This part is for the Hybrid GA-1, where 10% of the initial population is
%pooled with top routes from the NN algorithm solutions
...
%Intializing all with the top 10 NN routes
entry = 0;
for k = 2:pop_size
    entry = entry +1;
    if(entry > 10)
...
% This part is for the PureGA, where the initial population is randomly
% generated
...
% The best fitness is stored as the best_fitness for that generation
    ...
    title(['Best Hybrid GA-II Route (distance = ' num2str(best_fitness(iter)) ')'])
    figure(4)
    plot(best_fitness(1:iter), 'b', 'LineWidth', 2)
    title('Best Fitness - Hybrid GA-II')
    xlabel('Generation')
    ylabel('Distance')
